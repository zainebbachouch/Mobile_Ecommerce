...
public class CartAdapter extends RecyclerView.Adapter<CartAdapter.CartViewHodler> {
    ...
    public void onBindViewHolder(@NonNull CartViewHodler holder, int position) {
        ...
        // Load image with Glide and set size options
        Glide.with(holder.itemView.getContext())
                .load(shoeCart.getShoeImage()) // Assuming getShoeImage() returns the image URL or resource ID
                .override(500, 500) // Set the target width and height
                .into(holder.shoeImageView);

        // Remove the following line, as it's not needed when using Glide
